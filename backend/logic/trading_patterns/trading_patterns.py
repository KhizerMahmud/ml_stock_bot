...
def bull_flag(prices):
    ...
    """
    flagpole = prices['close'].iloc[-10:].pct_change().sum() > 0.1  # Strong upward trend
    consolidation = prices['close'].iloc[-5:].mean() < prices['close'].iloc[-10:-5].mean()  # Downward consolidation
    breakout = prices['close'].iloc[-1] > prices['close'].iloc[-5:].max()  # Breakout above flag
...
def bear_flag(prices):
    ...
    """
    flagpole = prices['close'].iloc[-10:].pct_change().sum() < -0.1  # Strong downward trend
    consolidation = prices['close'].iloc[-5:].mean() > prices['close'].iloc[-10:-5].mean()  # Upward consolidation
    breakout = prices['close'].iloc[-1] < prices['close'].iloc[-5:].min()  # Breakout below flag
    return flagpole and consolidation and breakout
...
def double_top(prices):
    ...
    return abs(resistance - second_peak) < 0.01 * resistance and prices['close'].iloc[-1] < neckline
...
def double_bottom(prices):
    ...
    return abs(support - second_trough) < 0.01 * support and prices['close'].iloc[-1] > neckline
...
def head_and_shoulders(prices):
    ...
    return head > left_shoulder and head > right_shoulder and prices['close'].iloc[-1] < neckline
...
def ascending_triangle(prices):
    ...
    higher_lows = all(prices['close'].iloc[i] > prices['close'].iloc[i - 1] for i in range(-10, -1))
...
def descending_triangle(prices):
    ...
    lower_highs = all(prices['close'].iloc[i] < prices['close'].iloc[i - 1] for i in range(-10, -1))
...
def symmetrical_triangle(prices):
    ...
    """
    lower_highs = all(prices['close'].iloc[i] < prices['close'].iloc[i - 1] for i in range(-10, -5))
    higher_lows = all(prices['close'].iloc[i] > prices['close'].iloc[i - 1] for i in range(-5, -1))
    breakout = prices['close'].iloc[-1] > prices['close'].iloc[-5:].max() or prices['close'].iloc[-1] < prices['close'].iloc[-5:].min()
...
def rising_wedge(prices):
    ...
    """
    higher_highs = all(prices['close'].iloc[i] > prices['close'].iloc[i - 1] for i in range(-10, -5))
    higher_lows = all(prices['close'].iloc[i] > prices['close'].iloc[i - 1] for i in range(-5, -1))
...
def falling_wedge(prices):
    ...
    """
    lower_highs = all(prices['close'].iloc[i] < prices['close'].iloc[i - 1] for i in range(-10, -5))
    lower_lows = all(prices['close'].iloc[i] < prices['close'].iloc[i - 1] for i in range(-5, -1))
...
def rectangle_pattern(prices):
    ...
    support = prices['close'].iloc[-10:].min()
    resistance = prices['close'].iloc[-10:].max()
    breakout = prices['close'].iloc[-1] > resistance or prices['close'].iloc[-1] < support
    return breakout
...
def pennant(prices, volume):
    ...
    consolidation = prices['close'].iloc[-10:].std() < prices['close'].iloc[-20:-10].std()
    breakout = prices['close'].iloc[-1] > prices['close'].iloc[-10:].max()
    return flagpole and consolidation and breakout
...
def triple_top(prices):
    ...
    return prices['close'].iloc[-1] < neckline and len([p for p in prices['close'].iloc[-15:] if abs(p - resistance) < 0.01 * resistance]) >= 3
...
def triple_bottom(prices):
    ...
    return prices['close'].iloc[-1] > neckline and len([p for p in prices['close'].iloc[-15:] if abs(p - support) < 0.01 * support]) >= 3