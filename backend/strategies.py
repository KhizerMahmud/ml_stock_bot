...
class TradingStrategies:
    def __init__(self, df):
        if not all(
            col in df.columns for col in ["Timestamp", "High", "Low", "Close", "Volume"]
        ):
            raise ValueError(
                "DataFrame must contain the columns: 'Timestamp', 'High', 'Low', 'Close', 'Volume'."
            )
        self.df = df
    ...
    def breakout_strategy(self):
        ...
        for i in range(1, len(self.df)):
            ...
            if (
                ...
                and self.df["Volume"].iloc[i]
                > self.df["Volume"].iloc[i - 1] * 1.5  # Volume surge
            ):
